...
/**
 * Spectral stops across the ribbon width — continuous VIBGYOR fan.
 * Dense sampling so adjacent hues blend; no hard white gaps between bands.
 */
const RIBBON_STOPS = [
  '#2a0a6e', // violet
  '#3b1fc4', // indigo
...
const DispersionRibbonMaterial = shaderMaterial(
  ...
  /* glsl */ `
    ...
    void main() {
      ...
      // Light turbulence — melt bands together without tearing into streaks
      float n1 = fbm(vec2(along * 2.2 + uTime * 0.04, vUv.x * 1.4));
      float n2 = fbm(vec2(along * 4.1 - uTime * 0.025, vUv.x * 2.6 + 7.0));
      float warp = (n1 - 0.5) * 0.16 + (n2 - 0.5) * 0.07;

      // Colour runs across the ribbon's width
      float band = clamp(vUv.x + warp * 0.28, 0.0, 1.0);
      vec3 col = spectrum(band);

      // Soft body with gentle edge feather — keep mid-band hues visible
      float edge = abs(vUv.x - 0.5) * 2.0;
      float widthFade = pow(smoothstep(1.0, 0.55, edge), 0.55);
      ...
      // Mild density variation — avoid mottling that reads as gaps
      float density = fbm(vec2(along * 2.6 + uTime * 0.03, vUv.x * 2.1 + 3.7));
      float mottle = mix(0.78, 1.12, density);

      float hotspot = fbm(vec2(along * 1.6 - uTime * 0.015, vUv.x * 1.1 + 21.0));
      hotspot = smoothstep(0.72, 0.96, hotspot);

      // Soft core — lifts mid-fan without punching a white hole
      float core = pow(clamp(1.0 - edge * 1.15, 0.0, 1.0), 2.6);

      float mask = widthFade * lengthFade * mottle;
      vec3 rgb = col * mask;

      rgb += vec3(1.0, 0.96, 0.92) * core * lengthFade * 0.28;
      rgb += col * hotspot * lengthFade * widthFade * 0.35;
